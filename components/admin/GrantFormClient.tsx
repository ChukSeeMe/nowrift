...
import { createGrant } from '@/app/admin/actions';

export function GrantFormClient() {
  ...
  const [geoScope, setGeoScope] = useState('uk');
  const [sectors, setSectors] = useState('ai');
  const [eligibilityTags, setEligibilityTags] = useState('startup, sme');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err.message || 'Error occurred during grant creation');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-display-xl text-off-white">Add Grant Manually</h1>
          <p className="text-body-l text-muted">Register a new funding opportunity in the database.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 rounded-xl border border-border bg-surface space-y-6">
        ...
        <div className="flex flex-col gap-2">
          ...
          <textarea
            ...
            rows={4}
            placeholder="A concise description of what funding is available and the program scope..."
...
export default GrantFormClient;
